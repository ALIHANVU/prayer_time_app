...
class _QiblaScreenState extends State<QiblaScreen>
  ...
  /// Сырое значение с датчика (градусы от севера, 0–360)
  double _heading = 0;
  ...
  void _startCompass() {
    ...
    // Если через 3 секунды данных нет — компас не работает
  ...
  Widget _buildCompass(AppStrings strings) {
    ...
    // ГЛАВНАЯ ЛОГИКА:
    //
    // heading = куда смотрит верх телефона (0=север, 90=восток...)
    // qiblaDirection = фиксированное направление на Каабу от севера
    //
    // Стрелка на Каабу = qiblaDirection - heading
    //   Если heading=200 и qibla=200 → стрелка=0° (вверх) = ты смотришь на Каабу!
    //
    // Диск компаса вращается на -heading
    //   Чтобы "С" (Север) всегда указывал на реальный север
    // ═══════════════════════════════════════════════════════

    final qiblaAngle = _qiblaDirection - _smoothHeading;

    // Проверка: смотрит ли ±15° от Кыблы
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // ─── Диск компаса (вращается ПРОТИВОПОЛОЖНО heading) ───
          Transform.rotate(
